...
namespace caffe {
		...
		CUDA_KERNEL_LOOP(index, nthreads) {
			...
			if (has_ignore_label_ && label_value == ignore_label_) {
				...
				if (label_value == 0)
				{
					...
					loss[index] = -pow(power_prob_data[ind], gamma) * log_prob_data[ind];
				}
				else
				{
					/*loss[index] = -2 * alpha * power_prob_data[ind] * log_prob_data[ind];*/
					loss[index] = -pow(power_prob_data[ind], 1) * log_prob_data[ind];
				}
				/*****************************************************************************/
		...
		const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
	{
		...
		FocalLossForwardGPU<Dtype> << <CAFFE_GET_BLOCKS(nthreads),
			...
			label, loss_data, outer_num_, dim, inner_num_, has_ignore_label_, ignore_label_, counts, alpha_, gamma_);
		...
		Dtype* counts)
	{
		...
		CUDA_KERNEL_LOOP(index, nthreads) {
			...
			if (has_ignore_label_ && label_value == ignore_label_) {
				...
				/*Dtype grad = 2 * (label_value == 0 ? 1 - alpha : alpha) *
					(-gamma * (power_prob_data[ind_i] / max(1 - prob_data[ind_i], eps))
					* log_prob_data[ind_i] * prob_data[ind_i]
					+ power_prob_data[ind_i]);*/
				Dtype grad = -(label_value == 0 ? gamma : 1) * (power_prob_data[ind_i] / max(1 - prob_data[ind_i], eps))
				* log_prob_data[ind_i] * prob_data[ind_i]
				+ power_prob_data[ind_i];
