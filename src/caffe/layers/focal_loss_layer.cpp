		...
		for (int i = 0; i < outer_num_; ++i) {
			for (int j = 0; j < inner_num_; j++) {
				...
				if (label_value == 0)
				{
					...
					loss -= pow(power_prob_data[index],gamma_) * log_prob_data[index];
				}
				else
				{
					/*loss -= 2 * alpha_ * power_prob_data[index] * log_prob_data[index];*/
					loss -= pow(power_prob_data[index], 1) * log_prob_data[index];
				}
				/*****************************************************************************/
		...
		if (propagate_down[0]) {
			...
			for (int i = 0; i < outer_num_; ++i) {
				for (int j = 0; j < inner_num_; ++j) {
					...
					/*Dtype grad = 2 * (label_value == 0 ? 1 - alpha_ : alpha_) *
						(-gamma_ * (power_prob_data[ind_i] / std::max(1 - prob_data[ind_i], eps))
						* log_prob_data[ind_i] * prob_data[ind_i]
						+ power_prob_data[ind_i]);*/
					Dtype grad = -(label_value == 0 ? gamma_ : 1) * (power_prob_data[ind_i] / std::max(1 - prob_data[ind_i], eps))
					* log_prob_data[ind_i] * prob_data[ind_i]
					+ power_prob_data[ind_i];
