...
void MatchBBox(const vector<NormalizedBBox>& gt_bboxes,
  ...
  match_indices->clear();
  match_indices->resize(num_pred, -1);		//返回每个default bbox匹配上的正样本的下标
  ...
  if (label == -1) {
	...
	/// @yhzheng share_location = ture 所有正样本被视为一类
	...
	/// @yhzheng 只匹配指定正样本类别，num_gt(ground truth的数量)也是指定类别的
  ...
  for (int i = 0; i < num_pred; ++i) {
    ...
    for (int j = 0; j < num_gt; ++j) {
      ...
      if (overlap > 1e-6) {
		 /// @yhzheng match_overlaps中存放的是每个prediction和最佳匹配的gt_box之间的JaccardOverlap，
		 /// 最佳匹配的gt_box的ID会存在match_indices中
  ...
  //@yhzheng 计算和查找每个gt_box最佳匹配的prediction，若有某个gt_box没有找到对应的匹配prediction，直接跳出
  //！！！默认每个gt_box都会有对应的最佳匹配prediction，跳出的地方应该捕获异常
  while (gt_pool.size() > 0) {
    // Find the most overlapped gt and cooresponding predictions.
    int max_idx = -1;
    int max_gt_idx = -1;
    float max_overlap = -1;		//保存单个gt_box和所有prediction最好的匹配JaccardOverlap
    for (map<int, map<int, float> >::iterator it = overlaps.begin();
         it != overlaps.end(); ++it) {
      int i = it->first;	//prediction bbox index
    ...
    if (max_idx == -1) {
      // Cannot find good match.???? 正常情况不会进入该分支，需要异常提示
      break;
    } else {
  ...
  switch (match_type) {
    ...
    case MultiBoxLossParameter_MatchType_PER_PREDICTION:
      ...
      for (map<int, map<int, float> >::iterator it = overlaps.begin();
		...
		//else不可能出现 前面已经做过匹配了，overlaps中的每个元素必有一个最佳的匹配
      }
      break;
    default:
...
void FindMatches(const vector<LabelBBox>& all_loc_preds,
  ...
  for (int i = 0; i < num; ++i) {
    ...
    if (!use_prior_for_matching) {
      ...
      const int label = -1;
	  /// @yhzheng 对default bboxs 和 gt_bboxes 进行匹配
