	...
	namespace Frcnn {
		...
		void FrcnnProposalLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
			...
			for (int batch_index = 0; batch_index < num; batch_index++) {
				...
				const int fg_begin = (2 * batch_index + 1)*total_anchor_num;
				const int bbox_begin = (4 * batch_index)*total_anchor_num;
				const int transform_bbox_begin = 4 * batch_index * rpn_pre_nms_top_n;
				...
				BBoxTransformInv<Dtype> << <caffe::CAFFE_GET_BLOCKS(retained_anchor_num), caffe::CAFFE_CUDA_NUM_THREADS >> >(
					retained_anchor_num, rpn_bbox, height, width, FrcnnParam::feat_stride,
					im_height, im_width, sorted_indices, anchors_, &transform_bbox_[transform_bbox_begin]);
				cudaDeviceSynchronize();
				/*-------------------------验证代码-------------------------*/
				//if (retained_anchor_num > 0) {
				//	std::ofstream  outfile;
				//	if (_access("bbox_score_.txt", 0) != -1) // 如果临时文件存在，删除！
				//		remove("bbox_score_.txt");
				//	outfile.open("bbox_score_.txt", ios::out | ios::app);
				//	vector<float> boxes(rpn_pre_nms_top_n * 4);
				//	vector<Dtype> scores(total_anchor_num);
				//	vector<int> indices(retained_anchor_num);
				//	cudaMemcpy(&boxes[0], &transform_bbox_[transform_bbox_begin], 4 * rpn_pre_nms_top_n * sizeof(float), cudaMemcpyDeviceToHost);
				...
				//	std::ofstream  outfile;
				//	if (_access("flags_.txt", 0) != -1) // 如果临时文件存在，删除！
				//		remove("flags_.txt");
				//	outfile.open("flags_.txt", ios::out | ios::app);
				//	outfile << retained_anchor_num << std::endl;
				//	for (int i = 0; i < retained_anchor_num; i++) {
				...
				SelectBoxByIndices << <caffe::CAFFE_GET_BLOCKS(selected_num), caffe::CAFFE_CUDA_NUM_THREADS >> >(
					selected_num, &transform_bbox_[transform_bbox_begin], selected_indices_, &transform_bbox_[transform_bbox_begin], sorted_scores, bbox_score_);
				cudaDeviceSynchronize();

				/*-------------------------验证代码-------------------------*/
				//if (selected_num > 0) {
				//	std::ofstream  outfile;
				//	if (_access("bbox_score_after_filter.txt", 0) != -1) // 如果临时文件存在，删除！
				//		remove("bbox_score_after_filter.txt");
				//	outfile.open("bbox_score_after_filter.txt", ios::out | ios::app);
				//	vector<float> boxes_after_filter(rpn_pre_nms_top_n * 4);
				//	vector<Dtype> scores_after_filter(retained_anchor_num);
				//	cudaMemcpy(&boxes_after_filter[0], &transform_bbox_[transform_bbox_begin], 4 * rpn_pre_nms_top_n * sizeof(float), cudaMemcpyDeviceToHost);
				//	cudaMemcpy(&scores_after_filter[0], bbox_score_, retained_anchor_num * sizeof(Dtype), cudaMemcpyDeviceToHost);// 3000
				//	outfile << selected_num << std::endl;
				//	for (int i = 0; i < selected_num; i++) {
				//		outfile << scores_after_filter[i] << " ";
				//		outfile << boxes_after_filter[4 * i] << " "
				//			<< boxes_after_filter[4 * i + 1] << " "
				//			<< boxes_after_filter[4 * i + 2] << " "
				//			<< boxes_after_filter[4 * i + 3] << std::endl;
				//	}
				//	outfile.close();
				//}
				/*-------------------------验证代码-------------------------*/
				...
				//batch_transform_bbox_.push_back(transform_bbox_2); //
			...
			int box_begin = 0;
 			for (size_t batch_index = 0; batch_index < batch_keep_num.size(); batch_index++) {
				...
				SelectBoxAftNMS << <caffe::CAFFE_GET_BLOCKS(keep_num), caffe::CAFFE_CUDA_NUM_THREADS >> >(
					...
					&transform_bbox_[rpn_pre_nms_top_n * batch_index * 4],
