...
void FrcnnAnchorTargetLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[1]->num(); i++) {
    ...
    if(gt_boxes[i][2]>im_width && gt_boxes[i][3]>im_height)
    {std::cout << i << "  : " << gt_boxes[i][0] << ", " << gt_boxes[i][1] << ", " << gt_boxes[i][2] << ", " << gt_boxes[i][3] << ", " << im_width << "," << im_height;}
    //CHECK(gt_boxes[i][2]<=im_width && gt_boxes[i][3]<=im_height);
    DLOG(ERROR) << "============= " << i << "  : " << gt_boxes[i][0] << ", " << gt_boxes[i][1] << ", " << gt_boxes[i][2] << ", " << gt_boxes[i][3] << ", " << im_width << "," << im_height;
  }
