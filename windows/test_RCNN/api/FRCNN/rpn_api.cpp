...
vector<boost::shared_ptr<Blob<float> > > Rpn_Det::predict(const vector<std::string> blob_names) {
  ...
  net_->Forward(&loss);
  clock_t ends = clock();
  std::cout <<"  predict_time="<<(double)(ends - start)/ CLOCKS_PER_SEC<< std::setprecision(5);
...
void Rpn_Det::predict(const cv::Mat &img_in, std::vector<caffe::Frcnn::BBox<float> > &results){
  ...
  //clock_t ends = clock();
  //std::cout <<"  mean_time="<<(double)(ends - start)/ CLOCKS_PER_SEC;

  //start = clock();
  cv::resize(img, img, cv::Size(), scale_factor, scale_factor);
  //ends = clock();
  //std::cout <<"  resize_time="<<(double)(ends - start)/ CLOCKS_PER_SEC;  
  ...
  //ends = clock();
  //std::cout <<"  preprocess_time="<<(double)(ends - start)/ CLOCKS_PER_SEC;  
 ...
 //ends = clock();
 //std::cout <<"  lastout_time="<<(double)(ends - start)/ CLOCKS_PER_SEC;  
}
