...
def readXML(xml_name, resize_width, resize_height):
...
imgList = "Data_0728/test.txt"
TPs = 0 # 正检
FPs = 0 # 误检
FNs = 0 # 漏检

for imgFile in open(imgList).readlines():  # 对于每个测试图片
    img_name = imgFile.strip().split('.jpg ')[0]
    xml_name = imgFile.strip().split('.jpg ')[1]
    image = caffe.io.load_image(img_name+'.jpg')
    true_boxes = readXML(xml_name, resize_width, resize_height);
